...
var _extensionInject = require("@reef-defi/extension-inject");

var _util = require("@reef-defi/util");

var _utilCrypto = require("@reef-defi/util-crypto");
...
const onReefInjectedPromise = () => new Promise(resolve => {
  ...
  document.addEventListener(_extensionInject.REEF_INJECTED_EVENT, listener, false);

  if ((0, _extensionInject.isInjected)(_extensionInject.REEF_EXTENSION_IDENT)) {
    document.removeEventListener(_extensionInject.REEF_INJECTED_EVENT, listener);
    resolve(true);
  }
...
function web3Enable(originName) {
  ...
  if ((0, _extensionInject.isInjectionStarted)(_extensionInject.REEF_EXTENSION_IDENT) && !(0, _extensionInject.isInjected)(_extensionInject.REEF_EXTENSION_IDENT)) {
  ...
  exports.web3EnablePromise = web3EnablePromise = (0, _util2.documentReadyPromise)(() => initCompat.then(() => getWindowExtensions(originName).then(values => {
    console.log('web3Enable EXTENSIONS=', values);
    return values.filter(value => !!value[1]).map(_ref3 => {
      let [info, ext] = _ref3;
...
async function web3AccountsSubscribe(cb) {
  ...
  const triggerUpdate = () => cb(Object.entries(accounts).reduce((result, _ref7) => {
    let [source, list] = _ref7;
    result.push(...mapAccounts(source, list, ss58Format));
    return result;
  }, []));

  const unsubs = (await web3EnablePromise).map(_ref8 => {
    let {
      accounts: {
        subscribe
      },
...
async function web3FromSource(source) {
  ...
  const found = source && sources.find(_ref9 => {
    let {
      name
    } = _ref9;
    return name === source;
