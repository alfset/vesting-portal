...
export const tokenMap = (scope, map) => {
...
export const nativeScript = (scope, script) => {
    let cslScript;
    const kind = script.kind;
    switch (kind) {
        case Cardano.NativeScriptKind.RequireSignature: {
            cslScript = scope.manage(NativeScript.new_script_pubkey(scope.manage(ScriptPubkey.new(scope.manage(Ed25519KeyHash.from_bytes(Buffer.from(script.keyHash, 'hex')))))));
            break;
        }
        ...
        case Cardano.NativeScriptKind.RequireNOf: {
            ...
            cslScript = scope.manage(NativeScript.new_script_n_of_k(scope.manage(ScriptNOfK.new(script.required, cslScripts3))));
            break;
        }
        case Cardano.NativeScriptKind.RequireTimeBefore: {
            cslScript = scope.manage(NativeScript.new_timelock_expiry(scope.manage(TimelockExpiry.new(scope.manage(BigNum.from_str(script.slot.toString()))))));
            break;
        }
        case Cardano.NativeScriptKind.RequireTimeAfter: {
            cslScript = scope.manage(NativeScript.new_timelock_start(scope.manage(TimelockStart.new(scope.manage(BigNum.from_str(script.slot.toString()))))));
            break;
        }
        default:
            throw new SerializationError(SerializationFailure.InvalidNativeScriptKind, `Native Script Type value '${kind}' is not supported.`);
    }
    return cslScript;
};
export const value = (scope, { coins, assets }) => {
...
export const txIn = (scope, core) => scope.manage(TransactionInput.new(scope.manage(TransactionHash.from_bytes(Buffer.from(core.txId, 'hex'))), scope.manage(BigNum.from_str(core.index.toString()))));
export const txOut = (scope, core) => {
    ...
    if (core.datum && core.datumHash)
        throw new SerializationError(SerializationFailure.InvalidDatum, 'Only one type of datum can be specified for a given output but both Datum hash and inline Datum were provided');
    if (core.datum) {
        cslTxOut.set_datum(scope.manage(Datum.new_data(scope.manage(PlutusData.from_bytes(Buffer.from(core.datum, 'hex'))))));
    }
    if (core.datumHash) {
        cslTxOut.set_datum(scope.manage(Datum.new_data_hash(scope.manage(DataHash.from_bytes(Buffer.from(core.datumHash, 'hex'))))));
    }
        ...
        switch (scriptType) {
            ...
            case Cardano.ScriptType.Plutus:
                if (core.scriptReference.version === Cardano.PlutusLanguageVersion.V1) {
                    cmlScript = scope.manage(Script.new_plutus_v1(scope.manage(PlutusV1Script.new(Buffer.from(core.scriptReference.bytes, 'hex')))));
                }
                else if (core.scriptReference.version === Cardano.PlutusLanguageVersion.V2) {
                    cmlScript = scope.manage(Script.new_plutus_v2(scope.manage(PlutusV2Script.new(Buffer.from(core.scriptReference.bytes, 'hex')))));
                }
                break;
            default:
                throw new SerializationError(SerializationFailure.InvalidScriptType, `Script Type value '${scriptType}' is not supported.`);
        }
        cslTxOut.set_script_ref(ScriptRef.new(cmlScript));
    }
    return cslTxOut;
};
export const utxo = (scope, core) => core.map((item) => scope.manage(TransactionUnspentOutput.new(txIn(scope, item[0]), txOut(scope, item[1]))));
...
export const txMetadatum = (scope, metadatum) => {
    if (metadatum === null)
...
export const txMetadata = (scope, blob) => {
...
export const txMint = (scope, mint) => {
...
export const getScripts = (scope, scripts) => {
    const nativeScripts = scope.manage(NativeScripts.new());
    const plutusV1Scripts = scope.manage(PlutusV1Scripts.new());
    const plutusV2Scripts = scope.manage(PlutusV2Scripts.new());
...
export const txAuxiliaryData = (scope, auxiliaryData) => {
    if (!auxiliaryData)
        return;
...
export const txBody = (scope, { inputs, outputs, fee, validityInterval, certificates, withdrawals, mint, collaterals, requiredExtraSignatures, scriptIntegrityHash, totalCollateral, collateralReturn, referenceInputs }, auxiliaryData) => {
    const cslOutputs = scope.manage(TransactionOutputs.new());
    for (const output of outputs) {
        cslOutputs.add(txOut(scope, output));
    }
    const cslBody = scope.manage(TransactionBody.new(txInputs(scope, inputs), cslOutputs, scope.manage(BigNum.from_str(fee.toString())), typeof validityInterval?.invalidHereafter !== 'undefined'
        ?
...
export const txWitnessBootstrap = (scope, bootstrap) => {
    const witnesses = scope.manage(BootstrapWitnesses.new());
    for (const coreWitness of bootstrap) {
        witnesses.add(scope.manage(BootstrapWitness.new(scope.manage(Vkey.new(scope.manage(PublicKey.from_bytes(Buffer.from(coreWitness.key, 'hex'))))), scope.manage(Ed25519Signature.from_hex(coreWitness.signature)), Buffer.from(coreWitness.chainCode || '', 'hex'), scope.manage(AddrAttributes.from_bytes(Buffer.from(coreWitness.addressAttributes || '', 'base64'))))));
    }
    return witnesses;
};
...
export const witnessSet = (scope, witness) => {
    ...
    for (const [vkey, signature] of witness.signatures.entries()) {
        ...
        const vkeyWitness = scope.manage(Vkeywitness.new(scope.manage(Vkey.new(publicKey)), scope.manage(Ed25519Signature.from_hex(signature))));
...
export const tx = (scope, { body, witness, auxiliaryData }) => {
    const txWitnessSet = witnessSet(scope, witness);
    return scope.manage(Transaction.new(txBody(scope, body, auxiliaryData), txWitnessSet, txAuxiliaryData(scope, auxiliaryData)));
};
