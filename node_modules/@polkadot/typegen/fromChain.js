...
import { generateDefaultConsts, generateDefaultErrors, generateDefaultEvents, generateDefaultQuery, generateDefaultRpc, generateDefaultTx } from "./generate/index.js";
import { HEADER, writeFile } from "./util/index.js";
...
function generate(metaHex, pkg, output, isStrict) {
  ...
  writeFile(path.join(process.cwd(), output, 'augment-api.ts'), () => [HEADER('chain'), ...['@polkadot/api/augment/rpc', ...['consts', 'errors', 'events', 'query', 'tx', 'rpc'].filter(key => !!key).map(key => `./augment-api-${key}`)].map(path => `import '${path}';\n`)].join(''));
  process.exit(0);
}
