...
function generate(metaHex, pkg, output, isStrict) {
  ...
  (0, _index2.writeFile)(_path.default.join(process.cwd(), output, 'augment-api.ts'), () => [(0, _index2.HEADER)('chain'), ...['@polkadot/api/augment/rpc', ...['consts', 'errors', 'events', 'query', 'tx', 'rpc'].filter(key => !!key).map(key => `./augment-api-${key}`)].map(path => `import '${path}';\n`)].join(''));
  process.exit(0);
}
